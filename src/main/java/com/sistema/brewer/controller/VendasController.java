...
import com.sistema.brewer.controller.validator.VendaValidator;
...
import com.sistema.brewer.security.UsuarioSistema;
import com.sistema.brewer.service.CadastroVendaService;
import com.sistema.brewer.session.TabelasItensSession;
...
public class VendasController {
	...
	@InitBinder("venda")//@InitBinder inicializa todos objetos
	public void inicializarValidador(WebDataBinder binder) {
		binder.setValidator(vendaValidator);
	}
	//carrega a pagina de venda
	@GetMapping("/nova")
	public ModelAndView nova(Venda venda) {
		ModelAndView mv = new ModelAndView("venda/CadastroVenda");
		
		//faz a verificação se o uid é null
		setUuid(venda);
		//matem os dados digitado na tela/junto com o javaScript
	...
	//salva o cadastro de enviarEmail
	...
	public ModelAndView enviarEmail(Venda venda, BindingResult result, RedirectAttributes attributes, @AuthenticationPrincipal UsuarioSistema usuarioSistema) {
		...
		mailer.enviar(venda);
		attributes.addFlashAttribute("mensagem", String.format("Venda n° %d salva com sucesso e e_mail enviado", venda.getCodigo()));
		return new ModelAndView("redirect:/vendas/nova");
	}
	...
	public ModelAndView alterarQuantidadeItens(@PathVariable("codigoCerveja") Cerveja cerveja, Integer quantidade, String uuid) {
	...
	@GetMapping
	public ModelAndView pesquisar(VendaFilter vendaFilter,  BindingResult result, @PageableDefault(size = 3) Pageable pageable, HttpServletRequest httpServletRequest) {
	...
	private void setUuid(Venda venda) {
		if (StringUtils.isEmpty(venda.getUuid())) {
			//UUID codigo que é gerado na sessão que é muito dificil ser repetido
